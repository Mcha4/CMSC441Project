...
int main(){
    ...
    while(count < iteration){
        ...
        matrix D(matrixSize, matrixSize);//32
        matrix E(matrixSize, matrixSize);//64
        matrix F(matrixSize, matrixSize);//128
        matrix G(matrixSize, matrixSize);//256
        ...
        dataTable.setData(count, 6, memoryBasic); //memoryBasic

        // Opeartions
        // Time
        
        start = clock();
        D.strassenMatrixk(&A, &B, 32);
        stop = clock();
        timeTaken = stop - start;        
        ...
        dataTable.setData(count, 7, memoryStarssen); //memoryStarssen
